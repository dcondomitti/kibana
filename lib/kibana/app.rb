module Kibana
  class App < Sinatra::Application
    ...
    get '/' do
      headers "X-Frame-Options" => "allow","X-XSS-Protection" => "0" if Configuration::Allow_iframed
    ...
    get '/api/search/:hash/?:segment?' do
      ...
      if Configuration::Highlight_results
      ...
      result.response['kibana']['default_fields'] = Configuration::Default_fields
      # Enable clickable URL links
      result.response['kibana']['clickable_urls'] = Configuration::Clickable_URLs
    ...
    get '/api/analyze/:field/trend/:hash' do
      limit = Configuration::Analyze_limit
      show  = Configuration::Analyze_show
      req           = ClientRequest.new(params[:hash])
    ...
    get '/api/analyze/:field/terms/:hash' do
      limit   = Configuration::Analyze_show
      req     = ClientRequest.new(params[:hash])
      fields = Array.new
      fields = params[:field].split(',,')
      query   = TermsFacet.new(req.search,req.from,req.to,fields)
      indices = Kelastic.index_range(req.from,req.to,Configuration::Facet_index_limit)
    ...
    get '/export/:hash/?:count?' do
      ...
      count = Configuration::Export_show
      # TODO: Make the count number above/below functional w/ hard limit setting
      # count = params[:count].nil? ? 20000 : params[:count].to_i
      sep   = Configuration::Export_delimiter

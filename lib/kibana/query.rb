...
module Kibana
  class Query
    ...
    def initialize(question, from = nil, to = nil)
      ...
      question = Configuration::Filter == "" ?
        question : "(#{question}) AND #{Configuration::Filter}"
      ...
      if (question == "*")
        ...
        @question = {
          "query_string" => {
            "default_operator" => Configuration::Default_operator,
            "default_field" => Configuration::Primary_field,
            "query" => question
          }
        }
      end
...
=end
  class SortedQuery < Query
    attr_accessor :query,:from,:to
    def initialize(question, from, to, offset = 0, size = Configuration::Per_page, field = "@timestamp", order = "desc")
  ...
  class HighlightedQuery < Query
    ...
    def initialize(question, from, to, offset = 0, size = Configuration::Per_page, field = "@timestamp", order = "desc")
      ...
      @query['highlight'] = {
        ...
        "fields" => { Configuration::Highlighted_field => { "fragment_size" => 9999 } }
      }
    end
  end
...
=end
  class TermsFacet < Query
    def initialize(question, from, to, field, limit = Configuration::Analyze_show)
      super(question, from, to)

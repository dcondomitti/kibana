  ...
  class Kelastic
    ...
    class << self
      def all_indices
        ...
        if Config.constants.include?("ElasticsearchTimeout")
          if Configuration::ElasticsearchTimeout != ''
            http.read_timeout = Configuration::ElasticsearchTimeout
          end
        end
      ...
      def index_range(from,to,limit = -1)
        if Configuration::Smart_index == true
        	index_pattern = "logstash-%Y.%m.%d"
        	if Configuration::Smart_index_pattern != ""
        	  index_pattern = Configuration::Smart_index_pattern
        	end
      ...
      def server
        list = Configuration::Elasticsearch
      ...
      def current_index
        if Configuration::Smart_index == true
          index_pattern = (Configuration::Smart_index_pattern.empty? ? "logstash-%Y.%m.%d" : Configuration::Smart_index_pattern)
      ...
      def run(url,query)
        ...
        o['kibana'] = {'per_page' => Configuration::Per_page}
      ...
      def index_path(index)
        if Configuration::Type != ''
          path = "http://#{Kelastic.server}/#{index}/#{Configuration::Type}"
        else
